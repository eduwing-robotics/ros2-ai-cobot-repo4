...
- [ ] Replace normal learned row-terminal playback with native continuous consumption, reusing LeRobot inference/chunk handling and FR5 execution ownership. Connect actual owner-produced evidence without a second playback product. The proposed grouped-ARM mode is not adopted; preserve historical readers rather than adding an unused runtime mode.
  - Native rolling ARM transport port is implemented under the same transport ownership slot; normal OneJob/LeRobot admission, executable-time binding, evidence consumption and gripper integration remain open. Focused `direnv exec . python3 -m unittest tests.data_factory.test_arm_stream tests.data_factory.test_motion_transport_execution`: 40 PASS / 0.539 s, including reentrant-guard rejection and native cancellation/result preservation. Source/CPU checks do not qualify physical execution or the installed controller's preemption behavior.
...
- [ ] Exercise the corrected normal consumer on a fresh bounded physical attempt, preserving diagnostic retention and separate Pick/release/reset outcome; do not treat grouped compilation or CPU success as physical qualification.
