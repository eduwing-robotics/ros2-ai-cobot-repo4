...
- [ ] Remove unnecessary same-held-gripper ARM row barriers with an explicit replay-compatible selection; verify all original knots/times, gripper ordering, collision sampling, partial consumed indices and sole-owner cancellation. This does not qualify concurrent ARM/gripper execution.
- [ ] Join the retained action result to the actual checked native terminal observation under existing deadlines; verify buffered pre-result samples, progressing completion, genuine mismatch, expiry, cancellation and exact witness consumption without resampling.
